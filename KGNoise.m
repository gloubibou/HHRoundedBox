...
{
    CGRect	rect	= {CGPointZero, kKGNoiseImageSize, kKGNoiseImageSize};
    NSImage *image	= [[[NSImage alloc] initWithSize:NSSizeFromCGSize(rect.size)] autorelease];
...
@synthesize noiseBlendMode = _noiseBlendMode;
...
#else
@synthesize alternateBackgroundColor = _alternateBackgroundColor;
...
#endif
{
    ...
    switch (self.gradientDirection) {
        case KGLinearGradientDirection0Degrees:
            startPoint	= CGPointMake(NSMinX(bounds), NSMidY(bounds));
            endPoint	= CGPointMake(NSMaxX(bounds), NSMidY(bounds));
            break;

        case KGLinearGradientDirection90Degrees:
            startPoint	= CGPointMake(NSMidX(bounds), NSMaxY(bounds));
            endPoint	= CGPointMake(NSMidX(bounds), NSMinY(bounds));
            break;

        case KGLinearGradientDirection180Degrees:
            startPoint	= CGPointMake(NSMaxX(bounds), NSMidY(bounds));
            endPoint	= CGPointMake(NSMinX(bounds), NSMidY(bounds));
            break;

        case KGLinearGradientDirection270Degrees:
        default:
            startPoint	= CGPointMake(NSMidX(bounds), NSMinY(bounds));
            endPoint	= CGPointMake(NSMidX(bounds), NSMaxY(bounds));
            break;
    }
...
#endif
{
    ...
    CGPoint gradCenter	= CGPointMake(round(NSMidX(bounds)), round(NSMidY(bounds)));
    CGFloat gradRadius	= sqrt(pow((bounds.size.height / 2), 2) + pow((bounds.size.width / 2), 2));
